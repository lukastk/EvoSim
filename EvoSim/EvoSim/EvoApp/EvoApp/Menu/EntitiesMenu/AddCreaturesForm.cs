...
namespace EvoApp.Menu.EntitiesMenu
{
	public class AddCreaturesForm : Dialogue
	{
		...
		public static AddCreaturesForm ShowDialogue(Window window, IWorld _world, CloseEvent closeFunction = null, string category = null, ISkinFile file = null)
		{
			...
			form.Initialize(_world, closeFunction, "Add Creatures", true, true, category, file);
			form.Show(window);
		...
		public override void AddedToContainer()
		{
			...
			var randomizeCheckBox = builder.AddCheckBoxField("Randomize the genome: ");
			...
			builder.AddResizableButtonField("OK", delegate(object sender)
			{
				...
				if (!addByClicksCheckBox.Checked)
				{
					for (int i = 0; i < amountField.Value; i++)
					{
						...
						clone.Rotation = Globals.Random.NextDouble() * Math.PI * 2;
					}
				}
				else
				{
